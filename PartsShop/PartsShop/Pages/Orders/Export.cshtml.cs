...
namespace PartsShop.Pages.Orders
{
    ...
    public class ExportModel : PageModel
    {
        ...
        public ExportModel(PartsShop.Data.ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> OnGetExport()
        {
            if (_context.Orders != null)
            {
                ...
                for (int i = 0; i < Order.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 6).Value = string.Join("\n", GetProducts(cart.CartParts));
                    worksheet.Cell(i + 2, 7).Value = cart.CartParts.Sum(x=> x.PartCount * x.Part.PartPrice) + "лв.";
                    worksheet.Cell(i + 2, 8).Value = order.LastModified_19118073;
                }
        ...
        private string GetProducts(List<CartPart> parts)
        {
            StringBuilder sb = new StringBuilder();
            foreach (var part in parts)
            {

                sb.AppendLine($"{part.Part.PartName} - Ед. Цена: {part.Part.PartPrice} лв.: {part.PartCount} броя");
            }
